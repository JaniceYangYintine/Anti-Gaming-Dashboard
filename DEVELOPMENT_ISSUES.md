...
## 19. Vercel Python 入口若命名成 `app.py`，會和 `backend/app` 套件衝突
### 問題
第一次部署到 Vercel 時，把根目錄 entrypoint 命名為 `app.py`，但 backend 本身也有 `backend/app` 套件。Vercel 載入時會優先吃到錯的模組路徑。

### 影響
- 線上 function 直接 crash
- 會出現類似 `No module named 'app.main'; 'app' is not a package` 的錯誤

### 解法
- 刪除根目錄 `app.py`
- 改用根目錄 `index.py`
- 在 `index.py` 先把 `backend/` 加進 `sys.path`
- 再由 `from app.main import app` 載入 FastAPI app

## 20. Neon 提供的 `DATABASE_URL` 與 SQLAlchemy driver 預期不一致
### 問題
Neon / Vercel environment 會提供 `postgresql://...` 連線字串，但目前專案是用 SQLAlchemy + `psycopg`，不是 `psycopg2`。

### 影響
- 線上 function 啟動時會因 driver 不符而 crash
- 即使 environment variable 存在，API 也無法真正連到資料庫

### 解法
- 在 `backend/app/db/session.py` 新增 `_normalize_database_url`
- 偵測到 `postgresql://` 時自動轉成 `postgresql+psycopg://`
- 保留本機與 Cloud SQL/Neon 環境共用同一套設定入口

## 21. Dashboard 與 Learner 曾把 health check 寫死到 localhost
### 問題
前端 API base 已支援本機與同網域切換，但 health check 仍直接 fetch `http://localhost:8000/health`。

### 影響
- 線上 Dashboard 會顯示 `Backend API 無法連線`
- 線上 Database 狀態會顯示 `未知`
- 使用者容易誤以為正式環境 API 或資料庫已中斷

### 解法
- 在 `frontend/app.js` 與 `frontend/learner.js` 改以 `API_BASE` 推導 `HEALTH_URL`
- 正式環境改走同網域 `/health`
- 本機 `5500/5501` 開發環境仍維持打 `localhost:8000`
...
## 32. 學員測驗平台按鈕體感延遲偏高
### 問題
學員測驗平台部署到 `Vercel + Neon` 後，使用者點擊 `開始學習 Session`、`標記已閱讀`、`啟用鏡頭`、鏡頭測試按鈕與 `送出測驗並完成 Session` 時，會感覺按鈕有明顯延遲，像是「按了但沒有立刻反應」。

### 影響
- demo 時容易讓人誤以為按鈕沒按到或前端卡住
- 使用者可能重複點擊，增加重送請求風險
- 即使後端實際正常，互動體驗仍顯得不夠流暢

### 原因
- learner 前端有多個操作會同步等待 API 完成後才更新畫面
- `startSession()` 在建立 session 後，還會依序等待其他 API 與事件送出
- `markCardRead()` 原本是等 `card_swiped` API 回來後才更新 UI
- `window.alert()` 會阻塞主執行緒，讓建立 session 後的感覺更卡
- 在雲端部署情境下，Vercel function 與 Neon 資料庫本來就會有基本網路往返延遲，若前端每一步都阻塞等待，卡感會被放大

### 解法
- 在 `frontend/learner.js` 新增 `setButtonBusy()`，讓按鈕在處理中時立即切到忙碌狀態與文案
- `startSession()` 改為：
  - 先建立 session 並更新主要 UI
  - 將非關鍵的 `loadSelectedLearnerPenaltyStatus()` 與 `session_started` 事件改成背景送出
- `markCardRead()` 改成 optimistic UI：
  - 先更新卡片完成狀態
  - 若 API 失敗再 rollback
- 移除 `startSession()` 內阻塞感很重的 `window.alert()`
- `startCameraMonitor()`、鏡頭測試按鈕與 `submitQuiz()` 都補上忙碌狀態，避免使用者以為沒反應
- 在 `handleError()` 中統一復原按鈕狀態，避免失敗後按鈕卡在 disabled 或 loading 文案

## 33. ML 輔助偵測容易被誤解成取代規則式主體
### 問題
加入 `LOGISTIC_REGRESSION_RISK` 與 `DECISION_TREE_RISK` 後，文件與 demo 口條若沒有說清楚，容易讓人誤以為系統已改成「機器學習為主體」，或誤以為模型已使用大量真實訓練資料完成正式訓練。

### 影響
- 合規場景中，若用 ML 直接作為高風險或懲罰主依據，會降低可解釋性與可稽核性。
- 老師或主管可能追問訓練集、測試集、真實 label 與模型可靠性。
- 若不說明 synthetic dataset 與 PoC 限制，會讓模型成果被誤解為正式生產模型。

### 解法
- 明確定義架構為「規則式為主體，機器學習為輔助」。
- 高風險仍由明確規則式門檻負責，例如異常速度、切頁分心、頁面停留不足、鏡頭離開與多人出現。
- `LOGISTIC_REGRESSION_RISK` 只列為中風險，目前是固定係數的邏輯回歸形式輔助評分器。
- `DECISION_TREE_RISK` 只列為中風險，目前是 synthetic dataset 訓練的 PoC 決策樹。
- `train_decision_tree_sklearn.py` 是本機訓練工具，使用 `scikit-learn` 做標準 train / validation / test 與 metrics；production 不安裝 `scikit-learn`，只讀 JSON artifact 做推論。
- 文件統一補充：未來需以真實 session 與主管審核結果作為 label，重新訓練正式模型。

## 34. Logistic Regression 初期缺少真實訓練資料，不能宣稱為正式訓練模型
### 問題
使用者希望加入 logistic regression 來輔助偵測異常，但目前專案尚未累積足夠真實 session、主管審核結果與標註 label。若直接宣稱已完成正式機器學習模型訓練，會和實際資料成熟度不一致。

### 影響
- Demo 時容易被追問訓練集大小、測試集切分、label 來源與模型準確率。
- 在合規場景中，若用未驗證模型直接做高風險裁決，會降低可解釋性與可信度。
- 若未說明目前是固定係數輔助評分，文件會讓人誤以為系統已經有真實資料訓練後的 logistic regression。

### 解法
- 將 `LOGISTIC_REGRESSION_RISK` 定義為「邏輯回歸形式的固定係數輔助評分器」。
- 使用現有可解釋特徵進行加權，例如完成時間、改答次數、切頁次數、低互動、鏡頭離開與多人出現。
- 使用 sigmoid 形式輸出 `ml_risk_score`，讓後續可以平滑銜接正式 logistic regression 係數。
- 命中結果只建立中風險 flag，不直接觸發高風險鎖定或模組凍結。
- 文件中明確標註：未來需用真實 session 與主管審核結果重新訓練係數。

## 35. Decision Tree 需要訓練集與驗證集，但真實 label 尚未成熟
### 問題
使用者希望加入決策樹，並確認是否應該先選取特徵、建立風險等級、產生幾千筆訓練集與驗證集。目前專案已有行為事件資料結構，但尚未有大量經主管審核確認的真實作弊/非作弊 label。

### 影響
- 沒有 label 時，無法訓練可以代表真實業務場景的正式模型。
- 若只用規則式結果當 label，模型可能只是學會複製既有規則，無法發現規則未涵蓋的複合型異常。
- 若資料切分不清楚，容易只看訓練集表現，造成過度樂觀。

### 解法
- 先定義可解釋特徵：完成時間、答錯題數、改答次數、切頁次數、焦點比例、互動密度、鏡頭離開時間與多人偵測。
- 新增 `generate_synthetic_ml_data.py` 產生 synthetic dataset，先支援 PoC 與 demo。
- 將資料切分成 train / validation / test：
  - train 用於建立樹。
  - validation 用於觀察深度、葉節點大小與門檻是否過度擬合。
  - test 用於最後檢查模型泛化表現。
- 讓 `DECISION_TREE_RISK` 只建立中風險 flag，避免 synthetic data PoC 直接成為懲罰主依據。
- 文件中保留下一步：累積真實 session 與主管審核 label 後，再重新訓練正式模型。

## 36. 同時做純 Python CART 與 scikit-learn 決策樹時，角色容易混淆
### 問題
專案中同時加入 `train_decision_tree.py` 與 `train_decision_tree_sklearn.py`，若沒有說明兩者分工，容易讓人以為做了兩套互相衝突的模型。

### 影響
- 開發者可能不知道 production 實際使用哪個 artifact。
- Demo 時容易被問到「為什麼要做兩個決策樹」。
- 若把 `scikit-learn` 放進正式 runtime，可能增加 Vercel serverless 部署大小、冷啟動時間與套件相容性風險。

### 解法
- 明確定義兩者角色：
  - `train_decision_tree.py`：純 Python CART PoC，用來展示決策樹原理與避免依賴大型 ML 套件。
  - `train_decision_tree_sklearn.py`：本機訓練工具，用標準 `scikit-learn DecisionTreeClassifier` 產生模型與 metrics。
- production 不直接執行訓練腳本，也不需要安裝 `scikit-learn`。
- 訓練完成後把模型轉成 JSON artifact，由 `backend/app/ml/decision_tree.py` 用純 Python 讀取與推論。
- 文件中強調「本機訓練、雲端輕量推論」是部署策略，不是兩套模型互相競爭。

## 37. JSON artifact 若沒有版本與解釋資訊，未來很難稽核
### 問題
決策樹模型若只輸出節點與門檻，未來很難知道它使用哪些特徵、由哪種資料產生、模型版本是什麼，也不利於主管理解為什麼產生 `DECISION_TREE_RISK`。

### 影響
- 模型更換後難以回溯舊 flag 是由哪版模型產生。
- Dashboard 或稽核文件難以說明 ML 判斷依據。
- 若未記錄 `dataset_kind`，synthetic PoC 可能被誤解成真實資料訓練模型。

### 解法
- JSON artifact 保留模型名稱、模型版本、特徵名稱、節點結構與 metadata。
- 推論結果回傳模型資訊、風險分數與 decision path。
- `DECISION_TREE_RISK` flag 的 metadata 可保留 ML 推論摘要，讓主管能看到命中理由。
- 文件中明確標示目前 `dataset_kind` 是 synthetic，未來正式訓練需更新模型版本與資料來源說明。
