...
本文件整理目前 Anti-Gaming 合規風險監控 Web App 的系統架構與風險判斷流程。圖中的流程以目前實作為準：不包含 email 通知，不包含鏡頭/畫面偵測主流程。
...
flowchart LR
  ...
  class sessionService,eventService,ruleService,flagService service;
...
flowchart TD
  ...
  learning["閱讀卡片 / 作答 / 互動"]
  collectEvents["寫入 session_events<br/>card_swiped / answer_changed / context_switch<br/>mouse_activity / keyboard_activity / page_dwell_summary"]
  ...
  inactive["停用規則<br/>EXCESSIVE_CONTEXT_SWITCH<br/>LONG_FACE_ABSENCE<br/>MULTIPLE_FACES_PRESENT<br/>不進入目前主流程"]
  ...
  class speed,blind,changes,lowInput,lowFocus rule;
